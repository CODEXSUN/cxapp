...
export async function migratePlanModule(db: Kysely<PlatformDatabase>) {
  await db.schema
    .createTable("app_plans")
    .ifNotExists()
    ...
    .addColumn("monthly_price", "double precision", (c) => c.notNull().defaultTo(0))
    .addColumn("annual_price", "double precision", (c) => c.notNull().defaultTo(0))
    .addColumn("limits_json", "json", (c) => c.notNull())
