...
const printCopyOptions: Array<{ label: string; value: PurchasePrintCopy }> = [
  { label: "Original", value: "original" },
  { label: "Duplicate", value: "duplicate" },
  { label: "Office Copy", value: "office-copy" }
];

type PurchaseEntryToolId = "assign" | "attachments" | "downloadPdf" | "email" | "tags" | "whatsapp";
...
export function PurchaseShowPage({
  ...
  const [emailAddress, setEmailAddress] = useState(purchase.supplierEmail);
  const [whatsappNumber, setWhatsappNumber] = useState(purchase.supplierPhone);
  ...
  useEffect(() => {
    setEmailAddress(purchase.supplierEmail);
    setWhatsappNumber(purchase.supplierPhone);
  }, [purchase.id, purchase.supplierEmail, purchase.supplierPhone]);
        ...
        <div className="mb-4 grid gap-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="order-1 flex flex-wrap items-center justify-end gap-2 sm:order-2">
              ...
              <Button
                disabled={!canEdit}
                title={canEdit ? "Edit purchase" : "Submitted purchases cannot be edited"}
                type="button"
                variant="outline"
          ...
          <div className="billing-mail-document min-w-0 overflow-x-auto">
            <div className="grid min-w-fit justify-center gap-6">
              {printCopies.map((copy) => (
                <div key={copy}>
                  <PurchasePrintDocument copy={copy} purchase={purchase} />
                </div>
              ))}
            </div>
          </div>
          <div className="h-fit space-y-3 pt-4 print:hidden xl:sticky xl:top-4">
                    ...
                    onClick={async () => {
                      if (tool.id === "downloadPdf") {
                        ...
                        try {
                          ...
                          recordActivity(
                            `Downloaded PDF for ${purchase.invoiceNumber} (queue #${result.job.jobId})`
                          );
                          ...
                          toast.error(
                            error instanceof Error ? error.message : "Purchase PDF download failed."
                          );
                        } finally {
                          ...
                          onKeyDown={(event) => {
                            if (event.key === "Enter") {
                              ...
                              addListValue(
                                ...
                                setAssigneeInput,
                                setAssignees,
                                (value) => `Assigned purchase to ${value}`
                              );
                            }
                          }}
