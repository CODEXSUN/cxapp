...
export function PurchaseProductQuickForm({
  initialValue,
  loading,
  onCancel,
  onSave,
  ...
  async function createOption(
    ...
    const created = await createPurchaseLookup(kind, payload).catch((error: unknown) => {
      setValidationError(
...
function taxRateLabel(record: PurchaseLookupRecord) {
    ...
    <label className="grid gap-2">
      ...
      <WorkspaceLookup
        ...
        onCreate={async (name) => {
          ...
          return { ...purchaseCommonOption(record), value: String(record.id) };
        }}
...
export function purchaseDecimalValue(value: string | number) {
