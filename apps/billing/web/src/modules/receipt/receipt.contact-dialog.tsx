...
import {
  createReceiptAddressType,
  createReceiptLocation,
  listReceiptAddressTypes,
  listReceiptContactTypes,
...
export function ReceiptContactDialog({
  initialValue,
  onCancel,
  onSave
}: {
  initialValue: ReceiptContactSavePayload;
  onCancel: () => void;
  onSave: (payload: ReceiptContactSavePayload) => Promise<void>;
}) {
  ...
  const types = useQuery({
    queryFn: listReceiptContactTypes,
    queryKey: ["billing", "receipt", "lookups", "contact-types"]
  });
  const addressTypes = useQuery({
    queryFn: listReceiptAddressTypes,
    queryKey: ["billing", "receipt", "lookups", "address-types"]
  });
  const countries = useQuery({
    queryFn: () => listReceiptLocations("countries"),
    queryKey: ["billing", "receipt", "lookups", "countries"]
  });
  const states = useQuery({
    queryFn: () => listReceiptLocations("states"),
    queryKey: ["billing", "receipt", "lookups", "states"]
  });
  const districts = useQuery({
    queryFn: () => listReceiptLocations("districts"),
    queryKey: ["billing", "receipt", "lookups", "districts"]
  });
  const cities = useQuery({
    queryFn: () => listReceiptLocations("cities"),
    queryKey: ["billing", "receipt", "lookups", "cities"]
  });
  const pincodes = useQuery({
    queryFn: () => listReceiptLocations("pincodes"),
    queryKey: ["billing", "receipt", "lookups", "pincodes"]
  });

  useEffect(() => {
    const customer = (types.data ?? []).find(
      (option) => option.label.trim().toLowerCase() === "customer"
    );
    if (!customer || form.typeId) return;
    setForm((current) => ({ ...current, typeId: customer.value, typeName: customer.label }));
  ...
  async function createLocation(kind: ReceiptLocationKind, name: string) {
    const dependency =
      kind === "states"
        ? form.countryId
        : kind === "districts"
    ...
    const created = await createReceiptLocation(kind, locationPayload(kind, name, form));
        ...
        <div className="grid gap-4">
          <label className="grid gap-2">
            ...
            <WorkspaceLookup
              ...
              onCreate={async (name) => {
                const created = await createReceiptAddressType(name);
...
function LocationLookup({
  ...
  label: string;
  loading: boolean;
  onCreate: (kind: ReceiptLocationKind, name: string) => Promise<ReceiptLookupOption | undefined>;
  onPick: (record: ReceiptLocationRecord) => void;
  options: ReceiptLocationRecord[];
  value: string;
}) {
  return (
    <label className="grid gap-2">
      ...
      <WorkspaceLookup
        ...
        onValueChange={(selected, option) => {
          const record =
            ((option as ReceiptLookupOption | undefined)?.record as
              ReceiptLocationRecord | undefined) ?? options.find((item) => item.id === selected);
...
function locationOption(record: ReceiptLocationRecord): ReceiptLookupOption {
  return {
    label: record.name || record.pincode || record.code,
    record: record as ReceiptLookupRecord,
    value: record.id
  };
}
function persistedOption(record: ReceiptLookupRecord): ReceiptLookupOption {
  return { label: record.name || record.code || record.id, record, value: String(record.id) };
}
function locationPayload(kind: ReceiptLocationKind, name: string, form: ReceiptContactSavePayload) {
...
function locationPatch(
  kind: ReceiptLocationKind,
  record: ReceiptLocationRecord,
  form: ReceiptContactSavePayload
) {
  const next = { ...form };
  if (kind === "states")
    Object.assign(next, {
...
export function emptyReceiptContact(name: string): ReceiptContactSavePayload {
  return {
    addressLine1: "",
    addressLine2: "",
    addressTypeId: "",
