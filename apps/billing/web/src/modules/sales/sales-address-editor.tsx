...
export function SaleAddressDialog({
  ...
  loading?: boolean;
  onCancel: () => void;
  onSave: (draft: SaleAddressDraft) => Promise<void> | void;
  title: string;
}) {
  ...
  async function createLocation(kind: SaleLocationKind, name: string) {
    ...
    const created = await createSaleLocation(
      kind,
      saleAddressLocationPayload(kind, name, form)
    ).catch((error: unknown) => {
        ...
        <div className="grid gap-4">
          <label className="grid gap-2">
            ...
            <WorkspaceLookup
              ...
              onCreate={async (name) => {
                const created = await createSaleAddressType(name).catch((error: unknown) => {
                  setSaveError(
