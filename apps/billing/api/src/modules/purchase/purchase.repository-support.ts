...
import type { PurchaseLineItem, PurchaseStatus } from "./purchase.types.js";

export type PurchaseDatabase = Record<string, never>;
export type PurchaseTransaction = Transaction<PurchaseDatabase>;

export type PurchaseHeaderRow = {
...
export function purchaseDatabase(databaseName: string) {
...
export function emptyPurchaseEway() {
  return { billDate: "", billNo: "", transport: "", vehicleNo: "" };
}

export function emptyPurchaseEinvoice() {
  return { ackDate: "", ackNo: "", irn: "", signedQr: "" };
}

export function parsePurchaseEway(value: string | null) {
  try {
...
export function parsePurchaseEinvoice(value: string | null) {
  try {
...
export async function internalPurchase(database: Kysely<PurchaseDatabase>, uuid: string) {
  const scope = currentBillingScope();
  const result = await sql<{ id: number; status: PurchaseStatus }>`
    SELECT id, status FROM billing_purchases WHERE uuid = ${uuid}
...
export async function existingIds(database: Kysely<PurchaseDatabase>, query: RawBuilder<unknown>) {
...
export function toPurchaseItem(row: PurchaseItemRow): PurchaseLineItem {
  return {
...
export function formatAddress(row: PurchaseHeaderRow, kind: "billing" | "shipping") {
