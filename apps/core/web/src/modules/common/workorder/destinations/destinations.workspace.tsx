...
import { destinationsQueryKey, useDestinations } from "./destinations.hooks";
import { DestinationsList } from "./destinations.list";
import {
  activateDestinations,
  createDestinations,
  deactivateDestinations,
  forceDeleteDestinations,
  updateDestinations
} from "./destinations.services";
import type { DestinationsRecord, DestinationsSavePayload } from "./destinations.types";
type PendingAction = { record: DestinationsRecord; type: "force-delete" | "restore" | "suspend" };
...
export function DestinationsWorkspace() {
  ...
  const [editing, setEditing] = useState<DestinationsRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useDestinations();
  const saveMutation = useMutation({
    mutationFn: (payload: DestinationsSavePayload) =>
      editing ? updateDestinations(editing.id, payload) : createDestinations(payload),
    onError: showError("Unable to save destination"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: destinationsQueryKey });
      toast.success(`Destination ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteDestinations(record.id)
        : type === "restore"
          ? activateDestinations(record.id)
          : deactivateDestinations(record.id),
    onError: showError("Unable to update destination"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: destinationsQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Destination force deleted"
          : action.type === "restore"
  ...
  return (
    <WorkspacePage
      ...
      description="Manage destinations for the current tenant database."
      technicalName="page.common.workorder.destinations.list"
      title="Destinations"
    >
      <WorkspaceFilters
        filterOptions={[
...
function DestinationsActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} destination?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
