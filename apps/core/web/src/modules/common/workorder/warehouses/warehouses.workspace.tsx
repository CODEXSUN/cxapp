...
import { warehousesQueryKey, useWarehouses } from "./warehouses.hooks";
import { WarehousesList } from "./warehouses.list";
import {
  activateWarehouses,
  createWarehouses,
  deactivateWarehouses,
  forceDeleteWarehouses,
  updateWarehouses
} from "./warehouses.services";
import type { WarehousesRecord, WarehousesSavePayload } from "./warehouses.types";
type PendingAction = { record: WarehousesRecord; type: "force-delete" | "restore" | "suspend" };
...
export function WarehousesWorkspace() {
  ...
  const [editing, setEditing] = useState<WarehousesRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useWarehouses();
  const saveMutation = useMutation({
    mutationFn: (payload: WarehousesSavePayload) =>
      editing ? updateWarehouses(editing.id, payload) : createWarehouses(payload),
    onError: showError("Unable to save warehouse"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: warehousesQueryKey });
      toast.success(`Warehouse ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteWarehouses(record.id)
        : type === "restore"
          ? activateWarehouses(record.id)
          : deactivateWarehouses(record.id),
    onError: showError("Unable to update warehouse"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: warehousesQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Warehouse force deleted"
          : action.type === "restore"
            ? "Warehouse restored"
            : "Warehouse suspended",
        { description: String(record.name) }
  ...
  return (
    <WorkspacePage
      ...
      description="Manage warehouses for the current tenant database."
      technicalName="page.common.workorder.warehouses.list"
      title="Warehouses"
    >
      <WorkspaceFilters
        filterOptions={[
...
function WarehousesActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} warehouse?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
