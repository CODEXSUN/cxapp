...
import type { StockRejectionTypesRecord } from "./stock-rejection-types.types";
...
export function StockRejectionTypesList({
  ...
  onRestore: (record: StockRejectionTypesRecord) => void;
  onSuspend: (record: StockRejectionTypesRecord) => void;
  records: StockRejectionTypesRecord[];
}) {
  const columns: ColumnDef<StockRejectionTypesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No stock rejection types found."
      isLoading={loading}
      minWidth="760px"
    />
  );
