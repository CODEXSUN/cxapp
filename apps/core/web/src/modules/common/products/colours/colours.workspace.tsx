...
import { coloursQueryKey, useColours } from "./colours.hooks";
import { ColoursList } from "./colours.list";
import {
  activateColours,
  createColours,
  deactivateColours,
  forceDeleteColours,
  updateColours
} from "./colours.services";
import type { ColoursRecord, ColoursSavePayload } from "./colours.types";
type PendingAction = { record: ColoursRecord; type: "force-delete" | "restore" | "suspend" };

export function ColoursWorkspace() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(100);
  const [editing, setEditing] = useState<ColoursRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useColours();
  const saveMutation = useMutation({
    mutationFn: (payload: ColoursSavePayload) =>
      editing ? updateColours(editing.id, payload) : createColours(payload),
    onError: showError("Unable to save colour"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: coloursQueryKey });
      toast.success(`Colour ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteColours(record.id)
        : type === "restore"
          ? activateColours(record.id)
          : deactivateColours(record.id),
    onError: showError("Unable to update colour"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: coloursQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Colour force deleted"
          : action.type === "restore"
            ? "Colour restored"
            : "Colour suspended",
        { description: String(record.name) }
  ...
  return (
    <WorkspacePage
      ...
      description="Manage colours for the current tenant database."
      technicalName="page.common.products.colours.list"
      title="Colours"
    >
      <WorkspaceFilters
        filterOptions={[
...
function ColoursActionDialog({
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} colour?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
