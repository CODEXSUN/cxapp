...
import type { ColoursRecord } from "./colours.types";
...
export function ColoursList({
  ...
  loading: boolean;
  onEdit: (record: ColoursRecord) => void;
  onForceDelete: (record: ColoursRecord) => void;
  onRestore: (record: ColoursRecord) => void;
  onSuspend: (record: ColoursRecord) => void;
  records: ColoursRecord[];
}) {
  const columns: ColumnDef<ColoursRecord>[] = [
    {
        ...
        >
          <span className="inline-flex items-center gap-2">
            <span
              ...
              style={{ backgroundColor: String(row.original.name) }}
            />
