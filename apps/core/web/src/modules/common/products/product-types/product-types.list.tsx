...
import type { ProductTypesRecord } from "./product-types.types";
...
export function ProductTypesList({
  ...
  loading: boolean;
  onEdit: (record: ProductTypesRecord) => void;
  onForceDelete: (record: ProductTypesRecord) => void;
  onRestore: (record: ProductTypesRecord) => void;
  onSuspend: (record: ProductTypesRecord) => void;
  records: ProductTypesRecord[];
}) {
  const columns: ColumnDef<ProductTypesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No product types found."
      isLoading={loading}
      minWidth="760px"
    />
  );
