...
import { brandsQueryKey, useBrands } from "./brands.hooks";
...
type PendingAction = { record: BrandsRecord; type: "force-delete" | "restore" | "suspend" };
...
export function BrandsWorkspace() {
  ...
  const [editing, setEditing] = useState<BrandsRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useBrands();
  const saveMutation = useMutation({
    mutationFn: (payload: BrandsSavePayload) =>
      editing ? updateBrands(editing.id, payload) : createBrands(payload),
    onError: showError("Unable to save brand"),
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteBrands(record.id)
        : type === "restore"
          ? activateBrands(record.id)
          : deactivateBrands(record.id),
    onError: showError("Unable to update brand"),
  ...
  return (
    <WorkspacePage
      ...
      description="Manage brands for the current tenant database."
      technicalName="page.common.products.brands.list"
      title="Brands"
    >
      <WorkspaceFilters
        filterOptions={[
...
function BrandsActionDialog({
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} brand?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
