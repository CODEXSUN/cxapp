...
import type { ProductCategoriesRecord } from "./product-categories.types";
...
export function ProductCategoriesList({
  ...
  loading: boolean;
  onEdit: (record: ProductCategoriesRecord) => void;
  onForceDelete: (record: ProductCategoriesRecord) => void;
  onRestore: (record: ProductCategoriesRecord) => void;
  onSuspend: (record: ProductCategoriesRecord) => void;
  records: ProductCategoriesRecord[];
}) {
  const columns: ColumnDef<ProductCategoriesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No product categories found."
      isLoading={loading}
      minWidth="760px"
    />
  );
