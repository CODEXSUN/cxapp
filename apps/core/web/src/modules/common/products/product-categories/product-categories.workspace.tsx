...
import { productCategoriesQueryKey, useProductCategories } from "./product-categories.hooks";
import { ProductCategoriesList } from "./product-categories.list";
import {
  activateProductCategories,
  createProductCategories,
  deactivateProductCategories,
  forceDeleteProductCategories,
  updateProductCategories
} from "./product-categories.services";
import type {
  ProductCategoriesRecord,
  ProductCategoriesSavePayload
} from "./product-categories.types";
type PendingAction = {
  record: ProductCategoriesRecord;
...
export function ProductCategoriesWorkspace() {
  ...
  const [editing, setEditing] = useState<ProductCategoriesRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useProductCategories();
  const saveMutation = useMutation({
    mutationFn: (payload: ProductCategoriesSavePayload) =>
      editing ? updateProductCategories(editing.id, payload) : createProductCategories(payload),
    onError: showError("Unable to save product category"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: productCategoriesQueryKey });
      toast.success(`Product category ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteProductCategories(record.id)
        : type === "restore"
          ? activateProductCategories(record.id)
          : deactivateProductCategories(record.id),
    onError: showError("Unable to update product category"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: productCategoriesQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Product category force deleted"
          : action.type === "restore"
            ? "Product category restored"
            : "Product category suspended",
        { description: String(record.name) }
      );
  ...
  return (
    <WorkspacePage
      ...
      description="Manage product categories for the current tenant database."
      technicalName="page.common.products.product-categories.list"
...
function ProductCategoriesActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} product category?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
