...
import { productGroupsQueryKey, useProductGroups } from "./product-groups.hooks";
import { ProductGroupsList } from "./product-groups.list";
import {
  activateProductGroups,
  createProductGroups,
  deactivateProductGroups,
  forceDeleteProductGroups,
  updateProductGroups
} from "./product-groups.services";
import type { ProductGroupsRecord, ProductGroupsSavePayload } from "./product-groups.types";
type PendingAction = { record: ProductGroupsRecord; type: "force-delete" | "restore" | "suspend" };

export function ProductGroupsWorkspace() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(100);
  const [editing, setEditing] = useState<ProductGroupsRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useProductGroups();
  const saveMutation = useMutation({
    mutationFn: (payload: ProductGroupsSavePayload) =>
      editing ? updateProductGroups(editing.id, payload) : createProductGroups(payload),
    onError: showError("Unable to save product group"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: productGroupsQueryKey });
      toast.success(`Product group ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteProductGroups(record.id)
        : type === "restore"
          ? activateProductGroups(record.id)
          : deactivateProductGroups(record.id),
    onError: showError("Unable to update product group"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: productGroupsQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Product group force deleted"
          : action.type === "restore"
            ? "Product group restored"
            : "Product group suspended",
        { description: String(record.name) }
      );
  ...
  return (
    <WorkspacePage
      ...
      description="Manage product groups for the current tenant database."
      technicalName="page.common.products.product-groups.list"
...
function ProductGroupsActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} product group?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
