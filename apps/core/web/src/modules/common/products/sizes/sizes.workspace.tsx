...
import { sizesQueryKey, useSizes } from "./sizes.hooks";
...
type PendingAction = { record: SizesRecord; type: "force-delete" | "restore" | "suspend" };

export function SizesWorkspace() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(100);
  const [editing, setEditing] = useState<SizesRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useSizes();
  const saveMutation = useMutation({
    mutationFn: (payload: SizesSavePayload) =>
      editing ? updateSizes(editing.id, payload) : createSizes(payload),
    onError: showError("Unable to save size"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: sizesQueryKey });
      toast.success(`Size ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteSizes(record.id)
        : type === "restore"
          ? activateSizes(record.id)
          : deactivateSizes(record.id),
    onError: showError("Unable to update size"),
  ...
  return (
    <WorkspacePage
      ...
      description="Manage sizes for the current tenant database."
      technicalName="page.common.products.sizes.list"
      title="Sizes"
    >
      <WorkspaceFilters
        filterOptions={[
...
function SizesActionDialog({
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} size?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
