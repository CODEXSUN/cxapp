...
import type { SizesRecord } from "./sizes.types";
...
export function SizesList({
  ...
  loading: boolean;
  onEdit: (record: SizesRecord) => void;
  onForceDelete: (record: SizesRecord) => void;
  onRestore: (record: SizesRecord) => void;
  onSuspend: (record: SizesRecord) => void;
  records: SizesRecord[];
}) {
  const columns: ColumnDef<SizesRecord>[] = [
    {
