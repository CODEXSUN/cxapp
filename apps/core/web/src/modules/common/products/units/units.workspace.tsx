...
import { unitsQueryKey, useUnits } from "./units.hooks";
...
type PendingAction = { record: UnitsRecord; type: "force-delete" | "restore" | "suspend" };
...
export function UnitsWorkspace() {
  ...
  const [editing, setEditing] = useState<UnitsRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useUnits();
  const saveMutation = useMutation({
    mutationFn: (payload: UnitsSavePayload) =>
      editing ? updateUnits(editing.id, payload) : createUnits(payload),
    onError: showError("Unable to save unit"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: unitsQueryKey });
      toast.success(`Unit ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteUnits(record.id)
        : type === "restore"
          ? activateUnits(record.id)
          : deactivateUnits(record.id),
    onError: showError("Unable to update unit"),
  ...
  return (
    <WorkspacePage
      ...
      description="Manage units for the current tenant database."
      technicalName="page.common.products.units.list"
      title="Units"
    >
      <WorkspaceFilters
        filterOptions={[
...
function UnitsActionDialog({
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} unit?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
