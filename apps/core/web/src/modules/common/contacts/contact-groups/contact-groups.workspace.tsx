...
import { ContactGroupsForm } from "./contact-groups.form";
import { contactGroupsQueryKey, useContactGroups } from "./contact-groups.hooks";
import { ContactGroupsList } from "./contact-groups.list";
import {
  activateContactGroups,
  createContactGroups,
  deactivateContactGroups,
  forceDeleteContactGroups,
  updateContactGroups
} from "./contact-groups.services";
import type { ContactGroupsRecord, ContactGroupsSavePayload } from "./contact-groups.types";
type PendingAction = { record: ContactGroupsRecord; type: "force-delete" | "restore" | "suspend" };

export function ContactGroupsWorkspace() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(100);
  const [editing, setEditing] = useState<ContactGroupsRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useContactGroups();
  const saveMutation = useMutation({
    mutationFn: (payload: ContactGroupsSavePayload) =>
      editing ? updateContactGroups(editing.id, payload) : createContactGroups(payload),
    onError: showError("Unable to save contact group"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: contactGroupsQueryKey });
      toast.success(`Contact group ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteContactGroups(record.id)
        : type === "restore"
          ? activateContactGroups(record.id)
          : deactivateContactGroups(record.id),
    onError: showError("Unable to update contact group"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: contactGroupsQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Contact group force deleted"
          : action.type === "restore"
            ? "Contact group restored"
            : "Contact group suspended",
        { description: String(record.name) }
      );
  ...
  return (
    <WorkspacePage
      ...
      description="Manage contact groups for the current tenant database."
      technicalName="page.common.contacts.contact-groups.list"
...
function ContactGroupsActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} contact group?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
