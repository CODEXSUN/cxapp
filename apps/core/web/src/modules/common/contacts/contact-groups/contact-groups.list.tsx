...
import type { ContactGroupsRecord } from "./contact-groups.types";

export function ContactGroupsList({
  loading,
  onEdit,
  onForceDelete,
  onRestore,
  ...
  loading: boolean;
  onEdit: (record: ContactGroupsRecord) => void;
  onForceDelete: (record: ContactGroupsRecord) => void;
  onRestore: (record: ContactGroupsRecord) => void;
  onSuspend: (record: ContactGroupsRecord) => void;
  records: ContactGroupsRecord[];
}) {
  const columns: ColumnDef<ContactGroupsRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No contact groups found."
      isLoading={loading}
      minWidth="760px"
    />
  );
