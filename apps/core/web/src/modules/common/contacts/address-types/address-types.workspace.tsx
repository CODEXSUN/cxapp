...
import { AddressTypesForm } from "./address-types.form";
import { addressTypesQueryKey, useAddressTypes } from "./address-types.hooks";
import { AddressTypesList } from "./address-types.list";
import {
  activateAddressTypes,
  createAddressTypes,
  deactivateAddressTypes,
  forceDeleteAddressTypes,
  updateAddressTypes
} from "./address-types.services";
import type { AddressTypesRecord, AddressTypesSavePayload } from "./address-types.types";
type PendingAction = { record: AddressTypesRecord; type: "force-delete" | "restore" | "suspend" };

export function AddressTypesWorkspace() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(100);
  const [editing, setEditing] = useState<AddressTypesRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useAddressTypes();
  const saveMutation = useMutation({
    mutationFn: (payload: AddressTypesSavePayload) =>
      editing ? updateAddressTypes(editing.id, payload) : createAddressTypes(payload),
    onError: showError("Unable to save address type"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: addressTypesQueryKey });
      toast.success(`Address type ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteAddressTypes(record.id)
        : type === "restore"
          ? activateAddressTypes(record.id)
          : deactivateAddressTypes(record.id),
    onError: showError("Unable to update address type"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: addressTypesQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Address type force deleted"
          : action.type === "restore"
            ? "Address type restored"
            : "Address type suspended",
        { description: String(record.name) }
      );
  ...
  return (
    <WorkspacePage
      ...
      description="Manage address types for the current tenant database."
      technicalName="page.common.contacts.address-types.list"
      title="Address Types"
    >
      <WorkspaceFilters
        filterOptions={[
...
function AddressTypesActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} address type?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
