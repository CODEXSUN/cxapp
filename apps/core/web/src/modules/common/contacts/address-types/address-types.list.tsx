...
import type { AddressTypesRecord } from "./address-types.types";

export function AddressTypesList({
  loading,
  onEdit,
  onForceDelete,
  onRestore,
  ...
  loading: boolean;
  onEdit: (record: AddressTypesRecord) => void;
  onForceDelete: (record: AddressTypesRecord) => void;
  onRestore: (record: AddressTypesRecord) => void;
  onSuspend: (record: AddressTypesRecord) => void;
  records: AddressTypesRecord[];
}) {
  const columns: ColumnDef<AddressTypesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No address types found."
      isLoading={loading}
      minWidth="760px"
    />
  );
