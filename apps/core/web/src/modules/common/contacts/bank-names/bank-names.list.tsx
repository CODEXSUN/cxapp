...
import type { BankNamesRecord } from "./bank-names.types";

export function BankNamesList({
  loading,
  onEdit,
  onForceDelete,
  onRestore,
  ...
  loading: boolean;
  onEdit: (record: BankNamesRecord) => void;
  onForceDelete: (record: BankNamesRecord) => void;
  onRestore: (record: BankNamesRecord) => void;
  onSuspend: (record: BankNamesRecord) => void;
  records: BankNamesRecord[];
}) {
  const columns: ColumnDef<BankNamesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No bank names found."
      isLoading={loading}
      minWidth="760px"
    />
  );
