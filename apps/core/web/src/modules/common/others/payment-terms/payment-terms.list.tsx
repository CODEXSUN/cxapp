...
import type { PaymentTermsRecord } from "./payment-terms.types";
...
export function PaymentTermsList({
  ...
  loading: boolean;
  onEdit: (record: PaymentTermsRecord) => void;
  onForceDelete: (record: PaymentTermsRecord) => void;
  onRestore: (record: PaymentTermsRecord) => void;
  onSuspend: (record: PaymentTermsRecord) => void;
  records: PaymentTermsRecord[];
}) {
  const columns: ColumnDef<PaymentTermsRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No payment terms found."
      isLoading={loading}
      minWidth="760px"
    />
  );
