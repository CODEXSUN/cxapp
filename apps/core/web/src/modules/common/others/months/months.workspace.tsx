...
import { monthsQueryKey, useMonths } from "./months.hooks";
...
type PendingAction = { record: MonthsRecord; type: "force-delete" | "restore" | "suspend" };
...
export function MonthsWorkspace() {
  ...
  const query = useMonths();
  const saveMutation = useMutation({
    mutationFn: (payload: MonthsSavePayload) =>
      editing ? updateMonths(editing.id, payload) : createMonths(payload),
    onError: showError("Unable to save month"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: monthsQueryKey });
      toast.success(`Month ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteMonths(record.id)
        : type === "restore"
          ? activateMonths(record.id)
          : deactivateMonths(record.id),
    onError: showError("Unable to update month"),
    ...
    return (query.data ?? []).filter(
      (record) =>
        ...
        (!term ||
          ...
          String(record.startDate).toLowerCase().includes(term) ||
          String(record.endDate).toLowerCase().includes(term))
    );
  ...
  return (
    <WorkspacePage
      ...
      description="Manage months for the current tenant database."
      technicalName="page.common.others.months.list"
      title="Months"
    >
      <WorkspaceFilters
        filterOptions={[
...
function MonthsActionDialog({
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} month?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
