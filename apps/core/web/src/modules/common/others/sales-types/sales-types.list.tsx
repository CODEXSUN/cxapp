...
import type { SalesTypesRecord } from "./sales-types.types";

export function SalesTypesList({
  loading,
  onEdit,
  onForceDelete,
  onRestore,
  ...
  loading: boolean;
  onEdit: (record: SalesTypesRecord) => void;
  onForceDelete: (record: SalesTypesRecord) => void;
  onRestore: (record: SalesTypesRecord) => void;
  onSuspend: (record: SalesTypesRecord) => void;
  records: SalesTypesRecord[];
}) {
  const columns: ColumnDef<SalesTypesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No sales types found."
      isLoading={loading}
      minWidth="760px"
    />
  );
