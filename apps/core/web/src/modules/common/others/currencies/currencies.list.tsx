...
import type { CurrenciesRecord } from "./currencies.types";
...
export function CurrenciesList({
  ...
  loading: boolean;
  onEdit: (record: CurrenciesRecord) => void;
  onForceDelete: (record: CurrenciesRecord) => void;
  onRestore: (record: CurrenciesRecord) => void;
  onSuspend: (record: CurrenciesRecord) => void;
  records: CurrenciesRecord[];
}) {
  const columns: ColumnDef<CurrenciesRecord>[] = [
    {
  ...
  return (
    <WorkspaceTable
      columns={columns}
      data={records}
      emptyState="No currencies found."
      isLoading={loading}
      minWidth="760px"
    />
  );
