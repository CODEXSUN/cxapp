...
import { currenciesQueryKey, useCurrencies } from "./currencies.hooks";
import { CurrenciesList } from "./currencies.list";
import {
  activateCurrencies,
  createCurrencies,
  deactivateCurrencies,
  forceDeleteCurrencies,
  updateCurrencies
} from "./currencies.services";
import type { CurrenciesRecord, CurrenciesSavePayload } from "./currencies.types";
type PendingAction = { record: CurrenciesRecord; type: "force-delete" | "restore" | "suspend" };
...
export function CurrenciesWorkspace() {
  ...
  const [editing, setEditing] = useState<CurrenciesRecord | null | undefined>(undefined);
  const [pendingAction, setPendingAction] = useState<PendingAction | null>(null);
  const query = useCurrencies();
  const saveMutation = useMutation({
    mutationFn: (payload: CurrenciesSavePayload) =>
      editing ? updateCurrencies(editing.id, payload) : createCurrencies(payload),
    onError: showError("Unable to save currency"),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: currenciesQueryKey });
      toast.success(`Currency ${editing ? "updated" : "created"}`, {
  ...
  const lifecycleMutation = useMutation({
    mutationFn: ({ record, type }: PendingAction) =>
      type === "force-delete"
        ? forceDeleteCurrencies(record.id)
        : type === "restore"
          ? activateCurrencies(record.id)
          : deactivateCurrencies(record.id),
    onError: showError("Unable to update currency"),
    onSuccess: async (record, action) => {
      await queryClient.invalidateQueries({ queryKey: currenciesQueryKey });
      toast.success(
        action.type === "force-delete"
          ? "Currency force deleted"
          : action.type === "restore"
            ? "Currency restored"
            : "Currency suspended",
        { description: String(record.name) }
    ...
    return (query.data ?? []).filter(
      (record) =>
        ...
        (!term ||
          ...
          String(record.symbol).toLowerCase().includes(term))
    );
  ...
  return (
    <WorkspacePage
      ...
      description="Manage currencies for the current tenant database."
      technicalName="page.common.others.currencies.list"
      title="Currencies"
    >
      <WorkspaceFilters
        filterOptions={[
...
function CurrenciesActionDialog({
  action,
  loading,
  onCancel,
  onConfirm
  ...
  return (
    <AlertDialog open={action !== null} onOpenChange={(open) => !open && onCancel()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{verb} currency?</AlertDialogTitle>
          <AlertDialogDescription>
            {destructive
