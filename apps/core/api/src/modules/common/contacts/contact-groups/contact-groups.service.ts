...
export class ContactGroupsService {
  constructor(private readonly repository = new ContactGroupsRepository()) {}
  list(filters: ContactGroupsListFilters = {}) {
    return this.repository.list(filters);
  }
  get(id: string) {
    return this.repository.find(id);
  }
  create(input: ContactGroupsSavePayload) {
    this.validate(input);
    return this.save(() => this.repository.create(input));
  }
  async update(id: string, input: ContactGroupsSavePayload) {
  ...
  private async mutable(id: string): Promise<ContactGroupsRecord> {
    const record = await this.repository.find(id);
    if (!record) throw AppError.notFound("Contact Groups record was not found.");
    return record;
  }
  private validate(input: ContactGroupsSavePayload) {
  ...
  private async save<T>(work: () => Promise<T>) {
    try {
      ...
      if (isDuplicate(error)) throw AppError.conflict("Contact Groups record already exists.");
      throw error;
    }
  }
}
