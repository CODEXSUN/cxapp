...
const idParamsSchema = z.object({ id: z.string().regex(/^\d+$/, "Colours ID must be numeric.") });
...
const coloursQuerySchema = z.object({ search: z.string().trim().optional() });
export async function registerColoursRoutes(app: FastifyInstance) {
  registerContractRoute(app, {
    ...
    method: "GET",
    schemas: { querystring: coloursQuerySchema, response: z.array(coloursSchema) },
    url: COLOURS_COLLECTION_PATH
  });
  registerContractRoute(app, {
    handler: async ({ params }) => required(await service.get(params.id)),
    method: "GET",
    schemas: { params: idParamsSchema, response: coloursSchema },
    url: `${COLOURS_COLLECTION_PATH}/:id`
  });
  registerContractRoute(app, {
    handler: async ({ body }) => required(await service.create(body)),
    method: "POST",
    schemas: { body: coloursPayloadSchema, response: coloursSchema },
    url: COLOURS_COLLECTION_PATH
  });
  registerContractRoute(app, {
    handler: async ({ body, params }) => required(await service.update(params.id, body)),
    method: "PUT",
    schemas: { body: coloursPayloadSchema, params: idParamsSchema, response: coloursSchema },
    url: `${COLOURS_COLLECTION_PATH}/:id`
  });
  registerContractRoute(app, {
