...
export class ProductTypesService {
  constructor(private readonly repository = new ProductTypesRepository()) {}
  list(filters: ProductTypesListFilters = {}) {
    return this.repository.list(filters);
  }
  get(id: string) {
    return this.repository.find(id);
  }
  create(input: ProductTypesSavePayload) {
    this.validate(input);
  ...
  private async mutable(id: string): Promise<ProductTypesRecord> {
    const record = await this.repository.find(id);
    if (!record) throw AppError.notFound("Product Types record was not found.");
    return record;
  }
  private validate(input: ProductTypesSavePayload) {
  ...
  private async save<T>(work: () => Promise<T>) {
    try {
      ...
      if (isDuplicate(error)) throw AppError.conflict("Product Types record already exists.");
      throw error;
    }
  }
}
