...
export class BrandsService {
  constructor(private readonly repository = new BrandsRepository()) {}
  list(filters: BrandsListFilters = {}) {
    return this.repository.list(filters);
  }
  get(id: string) {
    return this.repository.find(id);
  }
  create(input: BrandsSavePayload) {
    this.validate(input);
    return this.save(() => this.repository.create(input));
  }
  async update(id: string, input: BrandsSavePayload) {
  ...
  private async save<T>(work: () => Promise<T>) {
    try {
      ...
      if (isDuplicate(error)) throw AppError.conflict("Brands record already exists.");
      throw error;
    }
  }
}
