...
const idParamsSchema = z.object({ id: z.string().regex(/^\d+$/, "Units ID must be numeric.") });
...
const unitsPayloadSchema = z.object({
  name: z.string().trim(),
  isActive: z.boolean().default(true),
  sortOrder: z.number().int().min(0).default(1000)
});
const unitsQuerySchema = z.object({ search: z.string().trim().optional() });
export async function registerUnitsRoutes(app: FastifyInstance) {
  registerContractRoute(app, {
    ...
    method: "GET",
    schemas: { querystring: unitsQuerySchema, response: z.array(unitsSchema) },
    url: UNITS_COLLECTION_PATH
  });
  registerContractRoute(app, {
    handler: async ({ params }) => required(await service.get(params.id)),
    method: "GET",
    schemas: { params: idParamsSchema, response: unitsSchema },
    url: `${UNITS_COLLECTION_PATH}/:id`
  });
  registerContractRoute(app, {
    handler: async ({ body }) => required(await service.create(body)),
    method: "POST",
    schemas: { body: unitsPayloadSchema, response: unitsSchema },
    url: UNITS_COLLECTION_PATH
  });
  registerContractRoute(app, {
    handler: async ({ body, params }) => required(await service.update(params.id, body)),
    method: "PUT",
    schemas: { body: unitsPayloadSchema, params: idParamsSchema, response: unitsSchema },
    url: `${UNITS_COLLECTION_PATH}/:id`
  });
  registerContractRoute(app, {
    ...
    method: "POST",
    schemas: { params: idParamsSchema, response: unitsSchema },
    url: `${UNITS_COLLECTION_PATH}/:id/activate`
  });
  registerContractRoute(app, {
    handler: async ({ params }) => required(await service.setActive(params.id, false)),
    method: "POST",
    schemas: { params: idParamsSchema, response: unitsSchema },
    url: `${UNITS_COLLECTION_PATH}/:id/deactivate`
  });
  registerContractRoute(app, {
    handler: async ({ params }) => required(await service.forceDelete(params.id)),
    method: "DELETE",
    schemas: { params: idParamsSchema, response: unitsSchema },
    url: `${UNITS_COLLECTION_PATH}/:id/force`
  });
}
function required<T>(record: T | null): T {
  if (!record) throw AppError.notFound("Units record was not found.");
  return record;
}
