...
export class StylesService {
  constructor(private readonly repository = new StylesRepository()) {}
  list(filters: StylesListFilters = {}) {
    return this.repository.list(filters);
  }
  get(id: string) {
    return this.repository.find(id);
  }
  create(input: StylesSavePayload) {
    this.validate(input);
  ...
  private async save<T>(work: () => Promise<T>) {
    try {
      ...
      if (isDuplicate(error)) throw AppError.conflict("Styles record already exists.");
      throw error;
    }
  }
}
