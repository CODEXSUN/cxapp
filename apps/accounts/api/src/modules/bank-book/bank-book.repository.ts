...
export class BankBookRepository {
  async register(databaseName: string): Promise<BankBookRegister | null> {
    ...
    const accountResult = await sql<BankBookAccountRow>`
      ...
      WHERE a.company_id=${scope.companyId} AND a.financial_year_id=${scope.financialYearId}
        AND a.deleted_at IS NULL AND a.is_group=0 AND a.is_bank=1
      ORDER BY a.code
    `.execute(database);
    const accounts = accountResult.rows.map(toBankBookAccount);
    if (accounts.length === 0) return null;

    const accountIds = accounts.map((account) => account.accountId);
    const linesResult = await sql<BankBookLedgerRow>`
    ...
    const lines: BankBookRegisterLine[] = [];
  ...
  async findBankAccount(databaseName: string, uuid: string) {
    const database = await getAccountsDatabase(databaseName);
    const scope = currentAccountsScope();
    const result = await sql<BankBookAccountRow>`
      SELECT a.id, a.uuid, a.code, a.name, a.account_type, a.opening_balance
      FROM acc_accounts a
    ...
    return row ? toBankBookAccount(row) : null;
  }
  ...
  async nextEntryNumber(databaseName: string): Promise<string> {
    ...
    return `BR-${String(next).padStart(5, "0")}`;
  }
}

type BankBookAccountRow = {
  account_type: string;
  code: string;
  id: number;
  name: string;
...
function toBankBookAccount(row: BankBookAccountRow): BankBookAccount {
  return {
