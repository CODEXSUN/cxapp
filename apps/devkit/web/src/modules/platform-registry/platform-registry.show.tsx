...
export function PlatformRegistryModuleShow({
  ...
  busy: boolean;
  module: PlatformRegistryModuleNode;
  parentName: string;
  onBack: () => void;
  onRefresh: () => void;
  onToggle: () => void;
  onUpdate: (patch: Partial<PlatformRegistryModuleNode>) => Promise<unknown>;
}) {
  const [activeTab, setActiveTab] = useState("info");
  const saveRows = async (section: Section, rows: PlatformRegistryDocumentationRow[]) => {
...
function InfoTab({
  ...
  module: PlatformRegistryModuleNode;
  onSave: (rows: PlatformRegistryDocumentationRow[]) => Promise<void>;
  onUpdate: (patch: Partial<PlatformRegistryModuleNode>) => Promise<unknown>;
  parentName: string;
}) {
  return (
...
function InlineInfoCard({
  ...
  const rows: Array<{
    field?: keyof PlatformRegistryModuleNode;
    label: string;
    value: string;
  }> = [
...
function EditableRows({
  ...
  busy: boolean;
  onSave: (rows: PlatformRegistryDocumentationRow[]) => Promise<void>;
  rows: PlatformRegistryDocumentationRow[];
  title: string;
}) {
  const [editing, setEditing] = useState<PlatformRegistryDocumentationRow | null>(null);
  ...
  const startEdit = (row: PlatformRegistryDocumentationRow) => {
    setEditing(row);
    setAdding(false);
...
function PlanningNotes({
  ...
  busy: boolean;
  notes: PlatformRegistryPlanningNote[];
  onSave: (notes: PlatformRegistryPlanningNote[]) => Promise<void>;
}) {
  const [editing, setEditing] = useState<PlatformRegistryPlanningNote | null>(null);
  const [adding, setAdding] = useState(false);
